...
const DREAM_DATA = {
    ...
    // Dream moods with associated data
    moods: {
        happy: {
            color: "#FFD700",
    ...
    templates: {
        ...
        emotional: [
            "Your emotional state in this dream suggests {moodInterpretation}",
...
class DreamInterpretationEngine {
    static extractSymbols(description) {
        const symbols = [];
        const words = description.toLowerCase().split(/\s+/);
        
        for (const word of words) {
            if (DREAM_DATA.symbols[word]) {
                symbols.push({
                    word: word,
                    data: DREAM_DATA.symbols[word]
                });
            }
        }
    ...
    static generateInterpretation(description, mood, type) {
        const symbols = this.extractSymbols(description);
        const moodData = DREAM_DATA.moods[mood] || DREAM_DATA.moods.neutral;
        
        let interpretation = Utils.getRandomElement(DREAM_DATA.templates.general);
        ...
        interpretation += ` ${Utils.getRandomElement(DREAM_DATA.templates.guidance)}`;
    ...
    static generateEmotionalInsights(mood, description) {
        const moodData = DREAM_DATA.moods[mood] || DREAM_DATA.moods.neutral;
        const template = Utils.getRandomElement(DREAM_DATA.templates.emotional);
        
        return template.replace('{moodInterpretation}', moodData.interpretation.toLowerCase());
    }
}
...
window.commonDreams = DREAM_DATA.commonDreams;
window.dreamSymbols = DREAM_DATA.symbols;
window.dreamMoods = DREAM_DATA.moods;

// Export functions for backward compatibility
window.generateInterpretation = (description, mood, type) => 
    DreamInterpretationEngine.generateInterpretation(description, mood, type);

window.generateEmotionalInsights = (mood, description) => 
    DreamInterpretationEngine.generateEmotionalInsights(mood, description);

window.extractSymbols = (description) => 
    DreamInterpretationEngine.extractSymbols(description);

// Export new engine
window.DreamInterpretationEngine = DreamInterpretationEngine;
